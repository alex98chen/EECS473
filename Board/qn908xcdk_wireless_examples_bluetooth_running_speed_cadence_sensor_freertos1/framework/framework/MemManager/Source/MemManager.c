...
/* Boolean gPoolIdMatchingOrder is used to check if the pool ID is matching the increment order */
static bool_t gPoolIdMatchingOrder = FALSE;
...
memStatus_t MEM_Init(void)
{
    ...
    for (;;)
    {
        ...
        /* Loop check to get the top address of each pool, the pool ID will increments 1 and record
           gPoolIdMatchingOrder is TRUE if pool ID matching order*/
        if ((int16_t)pPools->poolId > prePoolId)
        {
            if ((int16_t)pPools->poolId == (prePoolId + 1))
            {
                gPoolIdMatchingOrder = TRUE;
                memPoolsTopAdd[pPoolInfo->poolId] = pPools;
            }
            else
            {
                gPoolIdMatchingOrder = FALSE;
            }
        }
        else if ((int16_t)pPools->poolId < prePoolId)
        {
            gPoolIdMatchingOrder = FALSE;
        }
        else
        ...
        prePoolId = (int16_t)pPools->poolId;
...
)
{
    ...
    pools_t *pPools = gPoolIdMatchingOrder ? memPoolsTopAdd[poolId] : memPools;
    listHeader_t *pBlock;
