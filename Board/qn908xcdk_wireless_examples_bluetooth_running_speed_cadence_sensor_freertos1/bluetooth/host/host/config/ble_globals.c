...
#define gActiveDevicesStorageSize_c      (604U)
#define gProcedureDataStorageSize_c      (288U)
...
#if defined(gGattClientSupported_d) && (gGattClientSupported_d)
...
#if defined(gGattClientSupported_d) && (gGattClientSupported_d)
    if ((uint32_t)gProcedureDataEntrySize * (uint32_t)gAppMaxConnections_c > sizeof(gProcedureDataStorage))
    {
        status = FALSE;
    }
#endif
