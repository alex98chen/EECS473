...
static rscsConfig_t rscsServiceConfig = {service_rsc,
                                        ...
                                        &mRscsUserData,
                                        gRscs_TopOfShoe_c};
static uint16_t cpHandles[] = { value_sc_ctrl_point, value_led_value, value_send_value };
...
static ledConfig_t ledServiceConfig = {service_led, 0};

static bccConfig_t bccServiceConfig = {service_bcc};
...
static void BleApp_Config()
{
    ...
    led_Start(&ledServiceConfig);
    bcc_Start(&bccServiceConfig);
...
static void BleApp_ConnectionCallback (deviceId_t peerDeviceId, gapConnectionEvent_t* pConnectionEvent)
{
    ...
    switch (pConnectionEvent->eventType)
    {
        case gConnEvtConnected_c:
        {
            ...
            led_Subscribe(peerDeviceId, &ledServiceConfig);
            bcc_Subscribe(peerDeviceId, &bccServiceConfig);
            /* UI */
            LED_StopFlashingAllLeds();
            Led1On();
            if (mRunningStatus)
        ...
        case gConnEvtDisconnected_c:
        {
            ...
            Rscs_Unsubscribe();
            led_Unsubscribe(&ledServiceConfig);
            bcc_Unsubscribe(&bccServiceConfig);
...
static void BleApp_GattServerCallback (deviceId_t deviceId, gattServerEvent_t* pServerEvent)
{
    ...
    switch (pServerEvent->eventType)
    {
        case gEvtAttributeWritten_c:
        {
            ...
            if (handle == value_led_value)
            {
            	led_write(&ledServiceConfig, &pServerEvent->eventData.attributeWrittenEvent);
            }
            if (handle == value_send_value)
            {
            	// TODO
            	bcc_Send(&bccServiceConfig, pServerEvent->eventData.attributeWrittenEvent);
            }
            else
            {
...
static void TimerMeasurementCallback(void * pParam)
{
    ...
    measurement.instantSpeed = 256 * ( 2 + 30 * (random & 0x03 ));
